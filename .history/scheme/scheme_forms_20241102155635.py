...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        expressions = Pair(signature.rest, expressions.rest)
        validate_formals(expressions.first)
        env.define(name, LambdaProcedure(expressions.first, expressions.rest, env))
        return name
        # END PROBLEM 10
    else:
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 11
    "*** YOUR CODE HERE ***"
    MuProcedure(formals, expressions.rest)
    env.define(formals.first, MuProcedure(formals, expressions.rest))
    return formals.first
    # END PROBLEM 11
