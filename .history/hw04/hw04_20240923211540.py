...
def balanced(m):
    ...
    if is_planet(m):
        ...
        left_mass, right_mass = total_mass(left_end) * length(left(m)), total_mass(right_end) * length(right(m))
        return left_mass == right_mass and balanced(left_end) and balanced(right_end)
...
def max_path_sum(t):
    ...
    """
    def max_path_sum_helper(t, path_sum):
        if is_leaf(t):
            return path_sum + label(t)
        else:
            return max(max_path_sum_helper(b, path_sum + label(t)) for b in branches(t))
    return max_path_sum_helper(t, 0)
    """
