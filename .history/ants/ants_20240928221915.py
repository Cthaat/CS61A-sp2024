...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        flag = self.place
        num = 0
        while flag.entrance is not None:
            if flag.bees and num <= self.upper_bound and num >= self.lower_bound:
                return random_bee(flag.bees)
            flag = flag.entrance
            num += 1
        return None
        # END Problem 3 and 4
...
# The WallAnt class
class WallAnt(Ant):
    """WallAnt prevents ants from entering a place."""
    name = 'Wall'
    food_cost = 4
    implemented = True  # Change to True to view in the GUI
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 8a
        "*** YOUR CODE HERE ***"
        if other.is_container or self.ant_contained is not None:
            return False
        return True
        # END Problem 8a
...
class QueenAnt(ThrowerAnt):
    ...
    def action(self, gamestate):
        ...
        "*** YOUR CODE HERE ***"
        super().action(gamestate)
        while self.place is not None:
            if self.place.ant is not None:
                if self.place.ant.is_container and self.place.ant.ant_contained is not None and not self.place.ant.ant_contained.is_doubled:
                    self.place.ant.ant_contained.double()
                    self.place.ant.ant_contained.is_doubled = True
                elif not self.place.ant.is_doubled:
                    self.place.ant.double()
                    self.place.ant.is_doubled = True
